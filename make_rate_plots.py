    ...
    for time_window in dt:
   	...
   	# 1) Get phase offset from allsky fit for good amp and baseline fits.
    	...
    	rates, new_rate_bins, rates_std, _ = phys.rebin_rate_rec(
        	        rate_rec, bins=rate_rebins, ignore_zero_runs=False) # this was True
    	...
    	plt.title("{:.2f}".format(time_window)+"s")
    	plt.legend(loc="best")
    	plt.savefig("plot_stash/rates/allsky_rate_bg_2012-2014_"+"{:.2f}".format(time_window)+".pdf")
    	plt.clf()
